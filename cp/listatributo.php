	...
	switch($success){
		...
		case '1':
			$alert = '<div class="alert alert-success alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha creado correctamente el atributo.
					  </div>';	
		break;
		case '2':
			$alert = '<div class="alert alert-success alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha modificado correctamente el atributo.
					  </div>';	
		break;
		case '3':
			$alert = '<div class="alert alert-info alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha eliminado correctamente el(los) atributo(s).
					  </div>';	
		break;
		case '4':
			$alert = '<div class="alert alert-info alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha activado correctamente el(los) atributo(s).
					  </div>';	
		break;
		case '5':
			$alert = '<div class="alert alert-warning alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha desactivado correctamente el(los) atributo(s), cuando desactiva elementos estos no se muestran en la parte principal de la página.
					  </div>';	
		break;				
	}
}
...
$lista_atributos = $temporal -> listar_atributos();
$clave='AgrAtr';
$clave2='ElimAtr';
$clave3='AcDcAtr';
$clave4='SortTableAtr';
...
?>
	<title>Listado De Los Atributos</title>
<?php
                  ...
                  <div class='notifications bottom-right'></div>
                    ...
                    <div class="col-lg-12">
                        <div class="table-responsive">
                          ...
                          <?php
                              foreach ($lista_atributos as $elemento) {
                                  ...
                                  else
                                  {
                                    ...
                                    {
                                      $img='img/visible.png';
                                      $funcion='Desactivar('.$elemento['id_atributo'].')';
                                      $class = 'nover';
                                    }
                                    else
                                    {
                                      $img='img/invisible.png';
                                      $funcion='Activar('.$elemento['id_atributo'].')';
                                      $class = 'ver';
                                    }
                                  }
                        ...
                        echo  '<tr>
                                <td>
                                  <input type="hidden" name="idorden" class="idorden" value="'.$elemento['id_atributo'].'">
                                  <input type="checkbox" id="'.$elemento['id_atributo'].'" name="id_atributo[]" value="'.$elemento['id_atributo'].'">
                                  <label for="'.$elemento['id_atributo'].'"><span></span></label>
                                </td>                    
                                <td><a href="formatributo.php?id_atributo='.$elemento['id_atributo'].'">'.$elemento['nombre_esp'].'</a></td>
                                <td class=" visible-lg visible-md">'.$handle.'<img class="manita '.$class.'" onclick="'.$funcion.'" id="temp'.$elemento['id_atributo'].'" src="'.$img.'"></td>
                              </tr>';
                      }
                        ?>
                        	</form>
...
<script>
	function Activar(id){
		...
		$.ajax({
			...
			url:"operaciones.php",
			data:"id="+id+"&operaciones=activar_atributo",
	...
	function Desactivar(id){
		...
		$.ajax({
			...
			contentType: "application/x-www-form-urlencoded",
			url:"operaciones.php",
			data:"id="+id+"&operaciones=desactivar_atributo",
			success:function(data){				
