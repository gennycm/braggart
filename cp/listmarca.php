	...
	switch($success){
		...
		case '1':
			$alert = '<div class="alert alert-success alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha creado correctamente la marca.
					  </div>';	
		break;
		case '2':
			$alert = '<div class="alert alert-success alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha modificado correctamente la marca.
					  </div>';	
		break;
		case '3':
			$alert = '<div class="alert alert-info alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha eliminado correctamente la(s) marca(s).
					  </div>';	
		break;
		case '4':
			$alert = '<div class="alert alert-info alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha activado correctamente la(s) marca(s).
					  </div>';	
		break;
		case '5':
			$alert = '<div class="alert alert-warning alert-dismissable">
  						<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
  						<strong>¡MUY BIEN!</strong> Se ha desactivado correctamente la(s) marca(s)., cuando desactiva elementos estos no se muestran en la parte principal de la página.
					  </div>';	
		break;				
	}
}
...
$temporal = new marca();
$lista_marcas = $temporal -> listar_marcas();
$clave='AgrMarca';
$clave2='ElimMarca';
$clave3='AcDcMarca';
$clave4='SortTableMarca';
$sort="marca";
$handle="";
...
?>
	<title>Listado De Las Marcas</title>
<?php
                  ...
                  <div class='notifications bottom-right'></div>
                    ...
                    <div class="col-lg-12">
                        <div class="table-responsive">
                          ...
                          <?php
                              foreach ($lista_marcas as $elemento) {
                                  ...
                                  else
                                  {
                                    ...
                                    {
                                      $img='img/visible.png';
                                      $funcion='Desactivar('.$elemento['id_marca'].')';
                                      $class = 'nover';
                                    }
                                    else
                                    {
                                      $img='img/invisible.png';
                                      $funcion='Activar('.$elemento['id_marca'].')';
                                      $class = 'ver';
                                    }
                                  }
                        ...
                        echo  '<tr>
                                <td>
                                  <input type="hidden" name="idorden" class="idorden" value="'.$elemento['id_marca'].'">
                                  <input type="checkbox" id="'.$elemento['id_marca'].'" name="id_marca[]" value="'.$elemento['id_marca'].'">
                                  <label for="'.$elemento['id_marca'].'"><span></span></label>
                                </td>                    
                                <td><a href="formmarca.php?id_marca='.$elemento['id_marca'].'">'.$elemento['nombre'].'</a></td>
                                <td class=" visible-lg visible-md">'.$handle.'<img class="manita '.$class.'" onclick="'.$funcion.'" id="temp'.$elemento['id_marca'].'" src="'.$img.'"></td>
                              </tr>';
                      }
                        ?>
                        	</form>
...
<script>
	function Activar(id){
		...
		$.ajax({
			...
			url:"operaciones.php",
			data:"id="+id+"&operaciones=activar_marca",
	...
	function Desactivar(id){
		...
		$.ajax({
			...
			contentType: "application/x-www-form-urlencoded",
			url:"operaciones.php",
			data:"id="+id+"&operaciones=desactivar_marca",
			success:function(data){				
